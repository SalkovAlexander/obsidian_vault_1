...
Использование инструментов, таких как Blender Geometry Nodes и houdini сильно упрощает и ускоряет процесс создания генераторов и как следствие ассетов для игровыз проектов. Процесс создания процедурного 3д контента в током случае выглядит следующим образом: 
...
### Houdini engine
Именно поэтому на рынке существует продукт который позволяет интегрировать генераторы созданные внутри Houdini в игровые движки Unity и Unreal а также в пакеты 3д моделирования Maya и 3ds max. Houdini engine позволяет разработчикам использующим Houdini получить удобный интерфейс который дает возможность создать инструмент процедурного моделирования в Houdini, а затем внутри игрового движка получить доступ ко всем параметрам инструмента, настроить эти параметры, сгенерировать модель и размесить ее на игровой сцене без необходимости заходить или даже открывать houdini.
...
На рисунке показано как выглядит официальная схема работы с Houdini и Houdini engine
### Проблема Houdini и Houdini engine
Поскольку в работе речь идет о разработке инструментария для игрового проекта разрабатываемого на игровом движке Unity, мы рассмотрим Houdini engine для Unity. Сама интеграция распространяется бесплатно на github и в unity asset store, однако это программное обеспечение обеспечивает лишь интерфейс внутри Unity для работы с Houdini, поэтому для работы с houdini engine в unity нужна лицензия Houdini   
***
###### Zero-Links
-
***
